...
def process(config,data_path,radius,max_num_nbr):
    ...
    #print(all_nbrs[0][9][2])
    #print(all_nbrs[0][8][3])
    #print(all_nbrs[1][0][0].coords.tolist())

    for i,nbr in enumerate(all_nbrs):
        x = list(map(lambda x: x[2].tolist(),nbr[:max_num_nbr]))
        #print(x)
    all_nbrs = [sorted(nbrs, key=lambda x: x[1]) for nbrs in all_nbrs]
    nbr_fea_idx, nbr_fea, nbr_name= [], [], []
    ...
    for i,nbr in enumerate(all_nbrs):
        ...
        if len(nbr) < max_num_nbr:
            ...
            nbr_fea.append(list(map(lambda x: x[0].coords.tolist(),
                                    nbr[:max_num_nbr])))
            nbr_name.append(list(map(lambda x: str(str(x[0]).split(' ')[-1]),
                                    nbr[:max_num_nbr])))
    #print(nbr_fea_idx)
    #print(nbr_fea)
    ...
    return material_id,lattice,atom_fea,nbr_fea_idx,nbr_distance,nbr_subtract,volume,nbr_name
##########
...
def main(data_dir, output_path,name_database ,cutoff,max_num_nbr,compress_ratio,chunk_size=10000):
    ...
    for n, chunk in enumerate(tqdm(py_.chunk(data_files[:int(compress_ratio*len(data_files))], chunk_size))):
        ...
        graph_nbr_name = []
        for file in chunk:
            material_id,lattice,atom_fea,nbr_fea_idx,nbr_distance,nbr_subtract,volume, nbr_name = process(config,file,cutoff,max_num_nbr)
            graph_lattice.append(lattice)
            graph_names.append(material_id[len(data_dir):])
            graph_nodes.append(atom_fea)
            #print(atom_fea)
            graph_edges.append((nbr_fea_idx,nbr_distance,nbr_subtract))
            #print(graph_edges)
            graph_volume.append(volume)
            graph_nbr_name.append(nbr_name)
            #print(volume)
        for name, lattice,nodes,neighbors,volume,nbr_name2 in tqdm(zip(graph_names,graph_lattice,graph_nodes, graph_edges,graph_volume,graph_nbr_name)):
            graphs = dict()
            detail = dict()
            detail['lattice'] = lattice
            detail['nodes'] = nodes
            detail['neighbors'] = neighbors
            detail['volume'] = volume
            detail['nbr_name'] = nbr_name2
            graphs[name] = detail
            print(name)
            np.savez_compressed(os.path.join(output_path,"{}.npz".format(name)), graph_dict=graphs)
            #time.sleep(10)
if __name__ == '__main__':
    ...
    main(data_dir = 'D:/material_project/data_cif/整理過沒有重複/val_cif_conventional_standard/',output_path= 'D:/material_project/cif_to_graph/npz/conventional_standard/val_cif_conventional_standard_cutoff_6_max_num_nbr_12',name_database='MP',cutoff= 6 ,max_num_nbr = 12 ,compress_ratio = 1)
